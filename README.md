...
5. **SSO 临时邮箱**（与 Webmail 共用 `WEBMAIL_ADDR`）：登录用户在 `/temp` 创建、查看和释放
   `temp:{sub}` 绑定的随机地址；自动 TTL GC。服务调用方可用 Keystone/SSO 签发的 Bearer key 经
   Sluice 调用固定 `DELETE /api/v1/temp-mailboxes`，Corvid 只读取网关注入的 subject/scope，不解析
   token。SMTP 对临时域只接收仍有效且已创建的完整地址，不启用 catch-all。
...
- 临时邮箱各自拥有独立 `mailboxes` 行，owner 为 `temp:{SSO subject}`；创建、列表和安全阅读在
  `/temp` Web UI 完成。删除 API 与 Webmail 共用同一 listener，必须经过 Sluice：

  ```http
  DELETE /api/v1/temp-mailboxes
  ...
  Sluice 校验 key 后注入 `X-Auth-Subject` 和包含精确 `corvid:temp-mail:delete` 的
  `X-Auth-Scope`；Corvid 不接受 body 中的 owner/subject。属于本人、已不存在、他人或永久邮箱均统一
  返回 `204`（仅本人临时邮箱会被删除），避免地址存在性 oracle；存储故障返回 `503`。
