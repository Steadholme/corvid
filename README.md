# Corvid — HOLDFAST 主权邮件服务

Corvid 是 HOLDFAST 栈的自建邮件服务，用 Rust 从零实现，**替换宿主机现有的 Postfix**。
单一进程内含四个协作部件，共享同一个 Postgres 数据层：
...
- `mailboxes(addr TEXT PK, owner_sub TEXT)`：地址 → HOLDFAST 身份 `sub`。启动时幂等 upsert 主邮箱
...
`scripts/alt_port_smoke.sh`（或手动）会构建 `holdfast/corvid:dev`，用一次性 Postgres + 真实 DKIM
...
```yaml
  corvid:
    build:
      context: ../corvid
    image: holdfast/corvid:dev
    environment:
      CORVID_STORE: postgres
      DATABASE_URL: ${CORVID_DATABASE_URL}   # postgres://holdfast:...@postgres:5432/corvid
...
2. **构建镜像**：`docker build -t holdfast/corvid:dev ./corvid`。
