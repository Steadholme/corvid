...
5. **SSO 临时邮箱 API**（与 Webmail 共用 `WEBMAIL_ADDR`）：不提供 Web UI。自动化客户端使用
   Keystone/SSO 签发的 Bearer key，经 Sluice 调用 `/api/v1/temp-mailboxes` 下的完整管理 API，
   创建、读取、续期和删除 `temp:{sub}` 绑定的随机地址；Corvid 只读取网关注入的 subject/scope，
   不解析 token，并自动执行 TTL GC。SMTP 对临时域只接收仍有效且已创建的完整地址，不启用
   catch-all；API 同样只收信，不提供发送、回复或转发能力。
...
- 临时邮箱各自拥有独立 `mailboxes` 行，owner 为 `temp:{SSO subject}`；临时邮箱仅提供 API，
  与 Webmail 共用同一 listener 和 owner 边界。API 必须经过 Sluice，并使用 Keystone 账户页生成、带精确
  ...
  读取对不存在、他人、永久或过期邮箱统一返回 `404`；续期可恢复本人已过期或 legacy-zero
  临时邮箱，并在恢复会超过配额时返回 `409`；删除统一返回 `204`，避免存在性 oracle。消息详情
  的 HTML 会再次净化并移除所有图片元素，避免远程 tracking pixel；附件需通过独立 endpoint
  主动下载。输入非法为 `400`，鉴权失败为 `401/403`，存储故障为 `503`。所有 API 响应均为
  `Cache-Control: private, no-store` 和 `Vary: Authorization`。
