...
5. **临时邮箱 API**（`TEMP_MAIL_API_ADDR`）：独立于 Webmail 的 HTTP listener，仅在
   `MAIL_SEND_TOKEN` 和 `TEMP_MAIL_DOMAINS` 都配置时启动。API 创建随机独立邮箱并签发 24 小时
   Bearer token；SMTP 对临时域只接收已创建的完整地址，不启用主域 catch-all。
...
| `src/temp_mail.rs` | 独立临时邮箱 API + HMAC token 签发/鉴权 |
...
| `MAIL_SEND_TOKEN` | 空 | 内部发送 API 的 Bearer key；也用于临时邮箱 token 的域隔离 HMAC |
| `TEMP_MAIL_API_ADDR` | 空 | 独立临时邮箱 API 监听；空表示完全禁用，不得与 `WEBMAIL_ADDR` 共用公网入口 |
...
- 临时邮箱各自拥有独立 `mailboxes` 行；`POST /api/new_address` 返回 `{address,jwt}`，随后以
  `Authorization: Bearer <jwt>` 调用 `GET /api/mails` 和 `GET /api/mail/{id}`。该 API 应仅绑定
  宿主机 loopback，不经过 `mail.w33d.xyz` 的 Sluice/Webmail 路由。
