...
async fn ctx() -> Arc<SmtpContext> {
    ...
    store
        .upsert_mailbox(&Mailbox { addr: MAILBOX.to_string(), owner_sub: "w33d".to_string() })
        .await
        .unwrap();
    Arc::new(SmtpContext { config, store, signer: None, tls_acceptor: None })
}
...
async fn deliver(ctx: &Arc<SmtpContext>, env_from: &str, data_lines: &[&str]) {
    ...
    assert!(s
        ...
        .text
        .starts_with("250"));
    assert!(s.handle_line("RCPT TO:<w33d@w33d.xyz>").await.text.starts_with("250"));
    assert!(s.handle_line("DATA").await.text.starts_with("354"));
    for line in data_lines {
        s.handle_line(line).await;
    }
    let done = s.handle_line(".").await;
    assert!(done.text.starts_with("250"), "delivery accepted: {}", done.text);
}
...
async fn rule_moves_matching_mail_into_folder_at_delivery() {
    let ctx = ctx().await;
    ctx.store
        .add_rule(&rule("from", "contains", "alice@example.com", "move", Some("Archive")))
        .await
        .unwrap();

    deliver(&ctx, "alice@example.com", &[
    ...
    let archived = ctx.store.list_folder(MAILBOX, "Archive", None, 10).await.unwrap();
    assert_eq!(archived.len(), 1, "matching mail delivered into Archive");
    assert_eq!(archived[0].subject, "Filed");
    let inbox = ctx.store.list_folder(MAILBOX, "INBOX", None, 10).await.unwrap();
...
async fn rules_apply_in_position_order_first_match_wins() {
    ...
    let inbox = ctx.store.list_folder(MAILBOX, "INBOX", None, 10).await.unwrap();
    assert_eq!(inbox.len(), 1, "first match (star) wins — the move rule never runs");
    assert!(inbox[0].starred, "star rule applied");
    assert!(ctx.store.list_folder(MAILBOX, "Trash", None, 10).await.unwrap().is_empty());
}
...
async fn discard_rule_drops_silently_and_disabled_rules_are_skipped() {
    ...
    deliver(&ctx, "spammer@junk.example", &[
    ...
    .await;
    assert_eq!(ctx.store.message_count(MAILBOX).await.unwrap(), 0, "discarded, never stored");

    // A disabled rule never fires — carol's mail is delivered normally.
    deliver(&ctx, "carol@example.com", &[
...
async fn markread_rule_presets_seen_flag() {
    ...
    ctx.store
        ...
        .await
        .unwrap();
    deliver(&ctx, "a@b.com", &["From: a@b.com", "To: w33d@w33d.xyz", "Subject: fyi", "", "x"]).await;
...
async fn auto_reply_queues_once_per_sender_per_24h() {
    ...
    deliver(&ctx, "alice@example.com", &["From: alice@example.com", "Subject: ping", "", "x"]).await;
    let due = ctx.store.due_outbound(now_secs() + 5, 10).await.unwrap();
    assert_eq!(due.len(), 1, "one auto-reply queued via the existing outbound path");
    assert_eq!(due[0].to_domain, "example.com");
    assert_eq!(due[0].rcpts, vec!["alice@example.com".to_string()]);
    assert_eq!(due[0].env_from, "", "null envelope sender (RFC 3834) prevents loops");
    ...
    deliver(&ctx, "alice@example.com", &["From: alice@example.com", "Subject: ping2", "", "x"]).await;
    assert_eq!(ctx.store.due_outbound(now_secs() + 5, 10).await.unwrap().len(), 1);
    assert_eq!(ctx.store.message_count(MAILBOX).await.unwrap(), 2, "both messages delivered");

    // A different sender gets their own auto-reply.
    deliver(&ctx, "bob@example.com", &["From: bob@example.com", "Subject: hi", "", "x"]).await;
    assert_eq!(ctx.store.due_outbound(now_secs() + 5, 10).await.unwrap().len(), 2);
}

#[tokio::test]
async fn auto_reply_suppressed_for_bulk_and_auto_mail() {
    let ctx = ctx().await;
    ctx.store.set_auto_reply(MAILBOX, true, "OOO", "away", 0).await.unwrap();

    deliver(&ctx, "list@example.com", &[
    ...
    deliver(&ctx, "w33d@w33d.xyz", &["From: w33d@w33d.xyz", "Subject: self", "", "x"]).await;
    // Empty return-path (a bounce): never answered.
    deliver(&ctx, "", &["From: mailer-daemon@example.com", "Subject: bounce", "", "x"]).await;

    assert!(
        ctx.store.due_outbound(now_secs() + 5, 10).await.unwrap().is_empty(),
        "no auto-reply for bulk/list/auto/self/null-path mail"
    );
    assert_eq!(ctx.store.message_count(MAILBOX).await.unwrap(), 5, "all mail still delivered");
}
...
async fn auto_reply_honours_expiry_and_disabled_state() {
    ...
    deliver(&ctx, "a@example.com", &["From: a@example.com", "Subject: 1", "", "x"]).await;
    assert!(ctx.store.due_outbound(now_secs() + 5, 10).await.unwrap().is_empty());
    ...
    ctx.store
        ...
        .await
        .unwrap();
    deliver(&ctx, "b@example.com", &["From: b@example.com", "Subject: 2", "", "x"]).await;
    assert!(ctx.store.due_outbound(now_secs() + 5, 10).await.unwrap().is_empty());
    ...
    ctx.store
        ...
        .await
        .unwrap();
    deliver(&ctx, "c@example.com", &["From: c@example.com", "Subject: 3", "", "x"]).await;
    assert_eq!(ctx.store.due_outbound(now_secs() + 5, 10).await.unwrap().len(), 1);
}
...
async fn signature_prefills_compose_and_reply() {
    ...
    state.store.set_signature(MAILBOX, "Cheers,\nw33d").await.unwrap();
    ...
    assert!(html.contains("\n\n--\nCheers,\nw33d"), "compose prefilled with signature");
    ...
    let resp = app(state.clone()).oneshot(req).await.unwrap();
    let html = body_string(resp).await;
    assert!(html.contains("&gt; Original line"), "quoted original present");
    assert!(html.contains("--\nCheers,\nw33d"), "signature appended to the reply draft");
...
#[tokio::test]
async fn settings_page_renders_all_three_sections() {
    let state = build_dev_state().await;
    let req = Request::builder()
        .uri("/settings")
        .header("x-auth-subject", "w33d")
...
async fn settings_rules_add_reorder_toggle_delete_roundtrip() {
    ...
    let resp = app(state.clone()).oneshot(post(form, cookie.clone())).await.unwrap();
    assert_eq!(resp.status(), StatusCode::SEE_OTHER, "rule added");
    let form = format!("csrf={token}&field=subject&op=equals&needle=digest&action=move&folder=Archive");
    app(state.clone()).oneshot(post(form, cookie.clone())).await.unwrap();
...
async fn settings_signature_and_autoreply_forms_persist() {
    ...
    let req = Request::builder()
        ...
        .body(Body::from(format!("csrf={token}&signature=Regards%2C%0Aw33d")))
        .unwrap();
    let resp = app(state.clone()).oneshot(req).await.unwrap();
    ...
    assert!(s.auto_reply_until > now_secs(), "expiry stored as a future epoch");
    ...
    assert!(html.contains(r#"value="2027-01-31""#), "date input round-trips");
