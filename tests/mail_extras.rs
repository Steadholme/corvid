...
async fn threading_groups_by_references_headers() {
    ...
    let threads = store.list_folder_threads(MAILBOX, "INBOX", None, 50).await.unwrap();
    assert_eq!(threads.len(), 2, "A+B collapse into one conversation; D is its own");
    ...
    assert!(threads[1].latest.subject.contains("Re: Project Alpha"), "latest snippet is the reply");

    // The conversation view lists both messages of the Alpha thread, oldest first.
    let convo = store.list_thread(MAILBOX, &threads[1].thread_id, 50).await.unwrap();
...
async fn threading_falls_back_to_normalized_subject_when_headers_absent() {
    ...
    deliver(&store, inbound("x@ex.com", "Notes", "From: x@ex.com\r\nSubject: Notes\r\n\r\na", 100)).await;
    deliver(&store, inbound("y@ex.com", "RE: notes", "From: y@ex.com\r\nSubject: RE: notes\r\n\r\nb", 200)).await;
    // A genuinely different subject stays separate.
    deliver(&store, inbound("z@ex.com", "Invoice", "From: z@ex.com\r\nSubject: Invoice\r\n\r\nc", 300)).await;

    let threads = store.list_folder_threads(MAILBOX, "INBOX", None, 50).await.unwrap();
    assert_eq!(threads.len(), 2, "same normalized subject groups; different subject splits");
    let notes = threads.iter().find(|t| t.count == 2).expect("the Notes conversation");
...
async fn threaded_view_and_conversation_render() {
    ...
    let raw_a = "From: alice@example.com\r\nSubject: Launch\r\nMessage-ID: <la@ex.com>\r\n\r\nfirst";
    let raw_b = "From: bob@example.com\r\nSubject: Re: Launch\r\nMessage-ID: <lb@ex.com>\r\nReferences: <la@ex.com>\r\n\r\nsecond";
    process_inbound(store.as_ref(), None, "w33d.xyz", "alice@example.com", inbound("alice@example.com", "Launch", raw_a, 100)).await.unwrap();
    process_inbound(store.as_ref(), None, "w33d.xyz", "bob@example.com", inbound("bob@example.com", "Re: Launch", raw_b, 200)).await.unwrap();
    ...
    assert!(html.contains(r#"class="pill thread-count">2<"#), "message count badge shows 2");
    assert!(html.contains("Re: Launch"), "latest snippet shown");
    assert!(html.contains(r#"href="/?folder=INBOX""#), "Messages toggle back to the flat view");

    // Follow the conversation link: both messages render in order.
    let threads = store.list_folder_threads(MAILBOX, "INBOX", None, 10).await.unwrap();
    ...
    assert!(html.contains(r#"class="pill thread-count">2<"#), "conversation shows its message count");
}
...
async fn identity_add_ownership_and_outbound_from() {
    ...
    let resp = app(state.clone()).oneshot(post("/settings/identities", &cookie, form)).await.unwrap();
    ...
    let req = Request::builder().uri("/compose").header("x-auth-subject", "w33d").header(header::COOKIE, cookie.clone()).body(Body::empty()).unwrap();
    let html = body_string(app(state.clone()).oneshot(req).await.unwrap()).await;
    assert!(html.contains(r#"name="identity""#), "From selector present");
    assert!(html.contains("HOLDFAST Info &lt;info@w33d.xyz&gt;"), "identity listed");

    // Sending as the owned identity sets the outbound From + envelope sender to it.
    let form = format!("csrf={token}&action=send&identity={idn_id}&to=friend%40example.com&subject=Hi&body=x");
    let resp = app(state.clone()).oneshot(post("/send", &cookie, form)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::SEE_OTHER);
    let due = state.store.due_outbound(now_secs() + 5, 10).await.unwrap();
    assert_eq!(due.len(), 1);
    assert_eq!(due[0].env_from, "info@w33d.xyz", "envelope sender is the identity");
    assert!(due[0].raw.contains("From: HOLDFAST Info <info@w33d.xyz>"), "From header is the identity");
    let sent = state.store.list_folder(MAILBOX, "Sent", None, 10).await.unwrap();
    assert_eq!(sent.len(), 1);
    assert_eq!(state.store.get_message(&sent[0].id).await.unwrap().unwrap().msg_from, "HOLDFAST Info <info@w33d.xyz>");
}
...
async fn send_rejects_unowned_identity() {
    ...
    state
        ...
        .unwrap();
    let (token, cookie) = mint_csrf(&state).await;
    let form = format!("csrf={token}&action=send&identity=si_foreign&to=friend%40example.com&subject=Hi&body=x");
    let resp = app(state.clone()).oneshot(post("/send", &cookie, form)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST, "cannot send as another mailbox's identity");
    assert!(state.store.due_outbound(now_secs() + 5, 10).await.unwrap().is_empty(), "nothing enqueued");
}
...
async fn contacts_harvest_suggest_ranking_and_scope() {
    let store = InMemoryStore::new();
    store.upsert_mailbox(&corvid::model::Mailbox { addr: MAILBOX.to_string(), owner_sub: "w33d".to_string() }).await.unwrap();
    // Deliver two messages from the same sender (frequency 2) + one from another.
    deliver(&store, inbound("Frequent <freq@example.com>", "one", "From: Frequent <freq@example.com>\r\nSubject: one\r\n\r\na", 100)).await;
    deliver(&store, inbound("Frequent <freq@example.com>", "two", "From: Frequent <freq@example.com>\r\nSubject: two\r\n\r\nb", 200)).await;
    deliver(&store, inbound("rare@example.com", "three", "From: rare@example.com\r\nSubject: three\r\n\r\nc", 300)).await;
    // A manual contact sorts ahead of harvested ones regardless of frequency.
    store.upsert_contact(MAILBOX, "vip@example.com", "VIP", true).await.unwrap();

    let all = store.suggest_contacts(MAILBOX, "example.com", 10).await.unwrap();
    assert_eq!(all.len(), 3, "self address is never harvested; three correspondents");
    ...
    assert!(store.suggest_contacts("alice@w33d.xyz", "example.com", 10).await.unwrap().is_empty());
}

#[tokio::test]
async fn contacts_suggest_endpoint_is_scoped_json() {
    let state = build_dev_state().await;
    state.store.upsert_contact(MAILBOX, "friend@example.com", "Friend", true).await.unwrap();
...
async fn label_create_assign_filter_and_remove() {
    ...
    // Create a label.
    let resp = app(state.clone()).oneshot(post("/settings/labels", &cookie, format!("csrf={token}&name=Receipts"))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::SEE_OTHER);
    ...
    // A message to label.
    let mut m = inbound("a@b.com", "A receipt", "From: a@b.com\r\nSubject: A receipt\r\n\r\nbody", 100);
    m.thread_id = "t1".to_string();
    state.store.store_message(&m).await.unwrap();

    // Assign the label via the read-view control.
    let resp = app(state.clone()).oneshot(post(&format!("/m/{}/labels", m.id), &cookie, format!("csrf={token}&op=add&label={lid}"))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::SEE_OTHER);
    let applied = state.store.labels_for_message(MAILBOX, &m.id).await.unwrap();
    assert_eq!(applied.len(), 1);
    assert_eq!(applied[0].name, "Receipts");

    // Label-filter view lists it.
    let by = state.store.list_by_label(MAILBOX, &lid, None, 10).await.unwrap();
    assert_eq!(by.len(), 1);
    assert_eq!(by[0].id, m.id);
    let req = Request::builder().uri(format!("/?label={lid}")).header("x-auth-subject", "w33d").body(Body::empty()).unwrap();
    let html = body_string(app(state.clone()).oneshot(req).await.unwrap()).await;
    assert!(html.contains("A receipt"), "labelled message listed in the label view");
    assert!(html.contains("Label:"), "label heading");

    // Remove it.
    let resp = app(state.clone()).oneshot(post(&format!("/m/{}/labels", m.id), &cookie, format!("csrf={token}&op=remove&label={lid}"))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::SEE_OTHER);
    assert!(state.store.labels_for_message(MAILBOX, &m.id).await.unwrap().is_empty());
}
...
async fn label_assign_denied_across_mailboxes() {
    ...
    state.store.add_label(&Label { id: "lbl_x".into(), mailbox: MAILBOX.into(), name: "Mine".into(), color: String::new() }).await.unwrap();
    ...
    let foreign = Message { mailbox: "alice@w33d.xyz".into(), ..foreign };
    state.store.store_message(&foreign).await.unwrap();

    // Store-level guard: assigning w33d's label to alice's message is a no-op.
    state.store.assign_label(MAILBOX, &foreign.id, "lbl_x").await.unwrap();
    assert!(state.store.labels_for_message(MAILBOX, &foreign.id).await.unwrap().is_empty());
}
...
async fn filter_rule_add_label_action_applies_at_delivery() {
    ...
    app(state.clone()).oneshot(post("/settings/labels", &cookie, format!("csrf={token}&name=Bills"))).await.unwrap();
    let lid = state.store.list_labels(MAILBOX).await.unwrap()[0].id.clone();
    let form = format!("csrf={token}&field=from&op=contains&needle=biller&action=label&label={lid}");
    let resp = app(state.clone()).oneshot(post("/settings/rules", &cookie, form)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::SEE_OTHER, "label rule added");

    // A Move/label rule without a real target is rejected.
    let bad = format!("csrf={token}&field=from&op=contains&needle=x&action=label&label=lbl_missing");
    let resp = app(state.clone()).oneshot(post("/settings/rules", &cookie, bad)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST, "label rule needs one of your labels");
    ...
    process_inbound(
        ...
        "w33d.xyz",
        "biller@example.com",
        inbound("biller@example.com", "Statement", "From: biller@example.com\r\nSubject: Statement\r\n\r\nbody", 100),
    )
    .await
    .unwrap();
    let inbox = state.store.list_folder(MAILBOX, "INBOX", None, 10).await.unwrap();
    assert_eq!(inbox.len(), 1);
    let applied = state.store.labels_for_message(MAILBOX, &inbox[0].id).await.unwrap();
    assert_eq!(applied.len(), 1, "the delivery-time rule tagged the message");
